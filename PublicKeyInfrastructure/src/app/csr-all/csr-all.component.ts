...
@Component({
  selector: 'app-csr-all',
  imports: [CommonModule],
  templateUrl: './csr-all.component.html',
  styleUrl: './csr-all.component.css'
})
export class CsrAllComponent implements OnInit {
  ...
  loadCsrs(): void {
    ...
    this.apiService.getAllCsrs().subscribe({
      next: (response) => {
        console.log('All CSRs received:', response);
        this.csrList = response;
        this.isLoading = false;
      },
      error: (error) => {
        console.error('Error loading all CSRs:', error);
